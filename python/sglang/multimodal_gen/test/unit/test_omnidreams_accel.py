...
"""CPU unit tests for the OmniDreams P0/P1/P2 acceleration code.

* P0: ``CUDAGraphWrapper`` input-staging logic + ``set_or_copy`` pointer
  stability (the CPU-checkable half; capture/replay needs CUDA).
* P1: LightTAE (TAEHV) checkpoint-key coverage, ``frames_to_trim`` math, and a
  decode-shape smoke (real ``lighttaew2_1.pth`` -- skipped if absent).
* P2: LightVAE (pruned Wan) checkpoint-key coverage + encode-shape smoke
...
# --------------------------------------------------------------------------- #
# P0: CUDAGraphWrapper staging logic                                          #
# --------------------------------------------------------------------------- #
